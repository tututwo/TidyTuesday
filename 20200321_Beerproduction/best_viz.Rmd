...
beer_geo %>%
  # why only 1,1 works??????????????????????????????????
  # without initial x and y, the axis are missing
  ggplot(aes(1, 1)) +
  geom_point(aes(colour = national_level, size = alcoholic_percent)) +
  # states that drink less than national_level
  geom_richtext(
    data = filter(beer_geo, national_level == "non_alcoholic"),
  ...
  # states that drink less than national_level
  geom_richtext( # unlike geom_text, geom_richtext can create markdown label, which means more special operations
    data = filter(beer_geo, national_level == "alcoholic"),
    aes(label = label, vjust = vjust),
    color = "white", # determines color of label text and label outline.
    fill = NA, # Default fill color of label background is white, nullifying it makes the yellow come outs
    label.colour = NA # overrides color, so the label outline disappears
  ) +
  # title?
  geom_richtext(
    data = beer_geo %>% filter(state == "MI"),
    aes(label = title),
  ...
  facet_geo(vars(state), grid = "us_state_grid2", label = "code") +
  scale_color_manual(
    values = c("grey70", "#f2ad22"),
    guide = F
  ) +
  scale_size(
    range = c(10, 20),
